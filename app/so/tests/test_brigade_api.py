...
class PrivateBrigadeApiTest(TestCase):
    ...
    def test_retrieve_brigade_list(self):
        ...
        Brigade.objects.create(title="name", area=sample_area(), shtab=sample_shtab())
        Brigade.objects.create(title="yname", area=sample_area(), shtab=sample_shtab())
    ...
    def test_create_brigade_successful(self):
        ...
        area = sample_area()
        shtab = sample_shtab()
        payload = {"title": "success", "area": area.id, "shtab": shtab.id}
        self.client.post(BRIGADE_URL, payload)
        exists = Brigade.objects.filter(title=payload["title"]).exists()
